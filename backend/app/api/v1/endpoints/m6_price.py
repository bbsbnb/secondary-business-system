...
async def list_price_recognition_sheets(
    ...
    """查询认质认价单列表"""
    query = db.query(BusinessForm).filter(BusinessForm.node_type == "M6")
    ...
    for f in forms:
        instance = db.query(ApprovalInstance).filter(
            ...
            ApprovalInstance.node_type == "M6"
        ).order_by(ApprovalInstance.version.desc()).first()
        
        # Calculate price deviation
        unit_price = float(f.form_data.get("unit_price", 0))
        budget_unit_price = float(f.form_data.get("budget_unit_price", 0))
        deviation_pct = round((unit_price - budget_unit_price) / budget_unit_price * 100, 2) if budget_unit_price > 0 else 0
...
async def get_price_recognition_sheet(
    form_id: int,
    db: Session = Depends(get_db)
):
    """获取认质认价单详情"""
    ...
    if not form:
        raise HTTPException(status_code=404, detail="认质认价单不存在")
...
async def create_price_recognition_sheet(
    req: dict,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """创建认质认价单"""
    if not req.get("material_name"):
    ...
    rec_no = f"RZ-{datetime.now().strftime('%Y%m%d')}-{str(uuid.uuid4())[:6].upper()}"
    ...
    doc = ProjectDocument(
        project_id=project_id,
        category="price_recognition",
        subcategory="认质认价",
        title=f"认质认价: {rec_no} - {req.get('material_name', '')[:50]}",
        file_path="",
        file_type="document",
    ...
    return {
        ...
        "rec_no": rec_no,
        "status": "submitted",
        "message": "认质认价单已创建并提交审批",
    }


@router.put("/{form_id}", response_model=dict)
async def update_price_recognition_sheet(
    form_id: int,
    req: dict,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    if not form:
        raise HTTPException(status_code=400, detail="只有草稿状态的认质认价单可编辑")
    ...
    return {"id": form.id, "form_data": form.form_data, "message": "认质认价单已更新"}
...
async def upload_attachment(
    ...
    node_dir = os.path.join(UPLOAD_DIR, "M6", str(form.project_id))
