...
from app.schemas import BusinessFormCreate, PaginatedResponse
...
async def list_visa_sheets(
    ...
    """查询签证单列表"""
    query = db.query(BusinessForm).filter(BusinessForm.node_type == "M8")
    ...
    for f in forms:
        instance = db.query(ApprovalInstance).filter(
            ...
            ApprovalInstance.node_type == "M8"
        ).order_by(ApprovalInstance.version.desc()).first()
        
        # Get amount from form data
        amount = f.form_data.get("amount", 0)
        visa_type = f.form_data.get("visa_type", "technical")
...
async def get_visa_sheet(
    ...
    if not form:
        raise HTTPException(status_code=404, detail="签证单不存在")
...
async def create_visa_sheet(
    ...
    visa_no = f"QS-{datetime.now().strftime('%Y%m%d')}-{str(uuid.uuid4())[:6].upper()}"
    
    form_data = {
        **req,
        "visa_no": visa_no,
        "submit_date": datetime.now().strftime('%Y-%m-%d'),
        "visa_type": req.get("visa_type", "technical"),
    }
    ...
    doc = ProjectDocument(
        ...
        category="visa",
        subcategory="签证单",
        title=f"签证单: {visa_no} - {req.get('description', '')[:50]}",
        file_path="",
        file_type="document",
...
async def update_visa_sheet(
    ...
    if not form:
        raise HTTPException(status_code=400, detail="只有草稿状态的签证单可编辑")
    ...
    return {"id": form.id, "form_data": form.form_data, "message": "签证单已更新"}
...
async def upload_attachment(
    ...
    node_dir = os.path.join(UPLOAD_DIR, "M8", str(form.project_id))
